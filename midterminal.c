...
int main()
{
  ...
  while (1)
  {
    ...
    if (strlen(commandInput) > 0)
      add_history(commandInput);
    if (!strcmp(commandInput, "ls"))
    ...
    else if (strlen(commandInput) == 0)
    {
      free(commandInput);
      continue;
    } // THIS WILL PREVENT A VERY ANNOYING CRASHING PROBLEM
    // THAT I WAS TRYING TO FIX FOR AN HOUR
    
    else
      with_pipe_execute(commandInput);
    free(commandInput);
  }
  return 0;
}
